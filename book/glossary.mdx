...
* **geminal.** denoting substitutent groups or atoms that are attached to the same atom in a molecule (comes from the Latin word _gemini_, meaning &lqduo;twins.&rdquo;
...
* **profile matrix.** a matrix <M>P</M> created from a set of <M>N</M> equal-length strings where <M math="P_{ij}"/> represents the number of times symbol <M math="S_i" /> appears at position <M>j</M>.
